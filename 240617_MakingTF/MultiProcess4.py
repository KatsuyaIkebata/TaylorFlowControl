...
def pump_operation(pump_number, port, log_queue, start_event, stop_event, other_pump_event):
    """シリンジポンプの操作を行う"""
    ser = serial.Serial(
        port=port,
        baudrate=115200,
...
if __name__ == '__main__':
    ...
    # シリンジポンプの操作プロセスの開始
    pump1_process = Process(target=pump_operation, args=('Pump 1', 'COM6', log_queue, start_event1, stop_event1, start_event2))
    pump2_process = Process(target=pump_operation, args=('Pump 2', 'COM7', log_queue, start_event2, stop_event2, start_event1))
